...
div#search-container {
	...
	margin: 0 0 10px 0;
	padding: 3px;
	background-color: rgba(0,199,212,.3);
}
div#search-memberId {
	display: <%= type == null || "member_id".equals(type) ? "inline-block" : "none" %>;  <%-- admin은 다른 url도 들어오기 떄문에 기본값 null처리해줌 --%>
}
div#search-memberName {
	display: <%= type == null || "member_name".equals(type) ? "inline-block" : "none" %>;
}
div#search-gender {
	display: <%= type == null || "gender".equals(type) ? "inline-block" : "none" %>;
}
</style>
<script>
window.load = (e) => {
