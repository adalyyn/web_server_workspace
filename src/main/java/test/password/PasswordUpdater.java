	...
	private void start() {
		...
		try( 
			...
			for(Member member : list)
				System.out.printf("memberId : %s, password : %s%n", member.getMemberId(), member.getPassword());
			
		} catch (SQLException e) {
			e.printStackTrace();
		}
		
		//2. 일괄 업데이트 (리스트문을 순회하면서 멤버객체당 각각 업데이트 하기)
		...
		close(conn);
		System.out.println("[비밀번호 일관갱신이 끝났습니다. ]");
