...
--============================
--web계정 생성
alter session set "_oracle_script" = true; --c##, C##으로 시작하지 않는 일반계정 이용
...
--페이징 쿼리 작성하기 (top-n분석)
--1. rownum을 이용하는 방식
--rownum을 새로 부여하려면, where조건절 변경 또는 inline view를 사용해야 한다.
--offset이 있는 경우 inline viw를 한번 더 써야하는 불편함이 있다. 
...
where
    rnum between 11 and 10;

--2. row_number 윈도우함수 이용하는 방식(결과적으로는 이 방식 이용)
-- numPerPage = 10 이라면
...
-- start = numPerPage * cPage - (?)
-- 답 : ((cPage - 1) * numPerPage) + 1
...
--검색된 회원수 조회
select count(*) from member; --전체대상

select count(*) from member where member_id like '%a%';  --where절이 추가로 처리되어야 할 부분이다. 
...
create table board (
    ...
    writer varchar2(20),    --로그인 후 글쓰기 가능하게 할 것임.
...
--alter table board add content varchar2(4000) not null;
...
create table attachment(
    no number,  --첨부파일의 넘버
    board_no number not null,   --어떤 게시글의 첨부파일인지
    --DB에 직접 첨부파일(이진데이터)을 관리하지 않음(가능하지만 안함), DB데이터가 비싸기 때문에
    --저장은 서버에 할 것이고, 파일의 메타정보(파일명)의 이름을 바꿔서 저장할 것임.
    --중복된 파일 덮어쓰기 방지 & 보안문제 & 인코딩문제(숫자로된 파일명으로 바꿀것임), 실제 저장된 파일명은 노출하지않고 DB에서만 관리할 것.
    original_filename varchar2(255) not null,  --업로드한 파일명
    renamed_filename varchar2(255) not null,  --실제저장된 파일명
    reg_date date default sysdate,
    constraint pk_attachment_no primary key(no),
    constraint fk_attachment_board_no foreign key(board_no) references board(no) on delete cascade  --게시글지우면 첨부파일도 삭제.
);
...
--select *from ( select  row_number() over(order by reg_date desc) rnum, b.*  from board b) b where rnum between 1 and 10;
